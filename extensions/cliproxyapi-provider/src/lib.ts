...
export async function fetchCodexModels(
	...
	// A truncated/invalid body is a failed refresh, not an empty catalog.
	...
	throw new Error("Invalid models response: expected a model array, models array, or data array.");
}
...
export async function resolveMappedModels(
	...
	// Keep a known-good catalog when the proxy temporarily reports no usable models.
	if (loaded.models.length === 0 && (loadModelsCache(agentDir, baseUrlInput)?.models.length ?? 0) > 0) {
		throw new Error("Empty models response; keeping the cached model list.");
	}
