...
Worker STOPPED/FAILED, Reviewer BLOCK, and Judge NEEDS_INPUT/BLOCKED can optionally report BLOCKER_KIND ENVIRONMENT/INVOCATION/REQUIREMENT/SAFETY. Ordinary code results and success omit it. Environment/invocation evidence enters same-role/ready-phase, same-round operator attention without automatic retry or guessed source repair. Attention stays quiet without proposing revisions; safe retries/clarifications need exact host approval and remaining budget. In-assignment SAFETY pauses without remediation authority; unrelated pre-existing out-of-scope safety concerns are notify-only. Integrity checks still precede handling, and dirty/unreviewed changes cannot be accepted. Final `environment` classification is non-mutating and consumes no code round; its verification successor still consumes the run budget. Wrong argv/manager/cwd is `manifest_error`.
...
## Whole-run revision regression coverage

`tests/unit/core/run-revision.test.ts` covers upstream integrated-in-run work plus a blocked dependent, graph-wide replacement, selective generation adoption, abandoned execution, confirmation dismissal, invalid graph and changed-checkout refusal, durable finish replay, and actual adapter tool/worker settlement. `tests/integration/reset.test.ts` covers destructive namespace ownership, merged integration refusal, interrupted deletion, revised graph topology, and repeat reset/restart cycles. All use temporary local Git repositories; no provider or live execution is required.

Check the complete sequence, not merely prompt strings: `revise_run` withholds all workers; edits affect only graph Markdown; exact host approval precedes cleanup; changed plans and their old/new downstream closure restart with fresh assignments, while unrelated proof-backed DONE plans retain their source changes and original evidence. Check independent completions both before and after an invalidated integration, dependency rewiring/removal, shared-context invalidation, conflict refusal, repeated revisions, and crash replay across integration publication and generation adoption. Final verification must bind the new exact tree; stale requests must not mutate a later generation. An invalid or dismissed proposal preserves old execution without restarting it. Test restart-time Markdown/lifecycle tampering and recovered-session tool restrictions. Normal `/herder-reset` must remain usable after prior successful resets without weakening its merge/foreign-worktree guards.

## Scope-controlled rounds and progress

Run `npm run test:herder -- extensions/herder/tests/unit/core/round-progress.test.ts` for deterministic helper coverage: persisted terminal grouping/order and delivery identity, actual self-reported setup/checks, interrupted/missing evidence, Judge-only bounded repair contracts, stable-ID cumulative exclusions and validated reopening, plan/generation/run isolation, and YOLO Implementer-only reporting. These tests use synthetic action records, not real `.herder` state, providers, or a new database.

Backend lifecycle checks must establish Reviewer → Judge after every completed normal review (including approval, round 3, and final RUN audit); three attempts with no fourth automatic mutation; new-run default `9 × N + 12` with no automatic increase to existing allocations; and unchanged YOLO skips plus mandatory exact-tree final gates. Excluded findings require different explicit Reviewer evidence and Judge confirmation of an introduced/worsened regression before reopening; NEW relabeling, stale evidence, pre-existing unrelated safety/design concerns, and advisory findings grant no repair authority. Explicit existing-bug assignments remain in scope. Malformed/operational/interrupted reports remain honest stops or unknown evidence.

Verify separately that authorized next-round repairs are exact and bounded with scope/budget approval distinct; accept-as-is records exceptions rather than check success; drop preserves the patch and leaves dependents blocked. Worker NOTES/RATIONALE stay short (done/checks/remaining), with no mandatory SUMMARY field. Helper and prompt tests alone do not prove manager enforcement or semantic review quality.
