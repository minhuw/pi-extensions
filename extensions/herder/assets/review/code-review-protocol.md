...
Use this protocol only as the root `plan-reviewer` for the frozen assignment supplied by the deterministic Run Manager. This file owns HOW review is performed: risk floors, child selection and ownership, discovery versus verification, materiality, unresolved work, evidence collection, adjudication, stops, and the distinction between prompt-policy counts and hard runtime caps. The Reviewer contract owns assignment authority, frozen proof, setup, source-preservation boundaries, and the exact terminal envelope; it is mandatory and is loaded alongside this protocol. Preserve Herder's finding ledger and three-round repair policy. Every completed normal-mode review goes to Judge, including APPROVE, all three attempts, and final RUN audit. Judge `DONE` requires original criteria and required checks; `REPAIR` supplies a binding `PASS_DOCUMENT` for the next exact bounded repair, never expanded scope. Round 3 remains fresh-context `RESCUE` by the existing Implementer. Operational ENVIRONMENT/INVOCATION failures enter same-round attention, not guessed code repair. Manager-proven operational rescue may use precise failure evidence with `PASS_DOCUMENT: none`, never an invented waiver. Exhaustion does not authorize a fourth automatic mutation.
...
- Review approved assignment obligations and introduced/worsened behavior against the compiled assignment, explicit repository rules, changed-code contracts, required checks, and demonstrated regressions. Suppress style, speculation, pre-existing defects outside the assignment, and unrelated improvement ideas; an explicit existing-bug assignment remains in scope. Plan V2 A rows and explicitly binding decisions govern requirements within Boundaries; the route is suggested. A routine fix satisfying acceptance and scope does not violate the plan merely by using another approach.

Read the compact prior-round summaries and cumulative exclusions before deciding scope. Retain stable finding IDs. An excluded finding cannot become repair authority by relabeling it NEW: reopening requires explicit Reviewer new evidence different from the excluded evidence and Judge confirmation of a newly introduced or worsened regression. Otherwise keep it excluded. Exclusions are generation-scoped; reconsidering changed requirements needs explicit user-approved contract authority, not a new obligation inferred from review. Keep existing NOTES/RATIONALE short (done, checks, remaining); no extra SUMMARY field or report boilerplate. Self-reported checks remain distinct from authoritative final exact-tree gates.
...
- independently check materiality; reject unsupported speculation, demonstrated false positives, style preferences, pre-existing behavior outside the assignment, unrelated work, and claims contradicted by the plan, with concise reasons;
...
- independently reopen and verify surviving merged claims, not repeat redundant whole audits; do not reopen excluded findings without different explicit Reviewer evidence and Judge confirmation of an introduced/worsened regression;
...
3. Normalize only statuses for existing findings plus genuinely new regressions in the repair delta. Preserve every existing finding ID. Do not reopen excluded findings without different explicit Reviewer evidence and Judge confirmation of an introduced/worsened regression; omission from a partial report is not resolution.
...
Every final code-defect blocker requires an exact location, concrete triggering scenario, and reproducible evidence or a failing check. PATCH_REGRESSION additionally requires the introducing/worsening hunk or commit; PLAN_REQUIREMENT may instead identify an explicitly assigned existing bug that remains unfixed. A confirmed issue blocks only when it is an evidence-complete P0/P1 `PLAN_REQUIREMENT` or `PATCH_REGRESSION`, a failed explicit acceptance criterion, a failed required acceptance gate, or a material scope violation. Confirmed P2/P3 findings remain advisory; `FOLLOWUP` and `INVALID` never block. Preserve the contract's three-round authority rules. Reviewer findings alone never authorize repair: Judge classifies all findings after every completed review, including approval and final audit. Only an in-scope approved-contract violation or introduced/worsened regression can authorize an exact repair. Manager-proven operational failure evidence grants no new product scope. Exhaustion goes to the existing durable serialized attention queue; no reviewer or Judge may invent a fourth automatic mutation.
