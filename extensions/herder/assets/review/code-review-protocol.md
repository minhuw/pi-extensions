...
Use this protocol only as the root `plan-reviewer` for the frozen assignment supplied by the deterministic Run Manager. It uses risk-based plan discovery and four-reviewer final aggregate audits, preserving Herder's finding ledger, three-round repair policy, and exact terminal envelope. Round 1 is quick Implementer–Reviewer; round 2 is Implementer–Reviewer with Judge only on nonapproval. Judge `DONE` closes the task; `REPAIR` supplies a binding `PASS_DOCUMENT` for round-3 `RESCUE` by the existing fresh-context Implementer with unchanged tools and the profile's optional rescue binding (otherwise its normal Implementer binding), then independent Reviewer. The manager may also advance round 2 to `RESCUE` for manager-proven operational failures or conflicts without any prior Reviewer or Judge (`PASS_DOCUMENT: none`); use the unchanged original assignment and precise manager-supplied failure evidence, never an invented waiver. No round-3 Judge or fourth automatic mutation is allowed.
...
- Never use `worker`, edit source or plans, commit, or integrate. Reviewers have unrestricted bash and `readOnly: false`: source preservation is a behavioral contract, not a sandbox. The parent may perform repository-declared locked/pinned setup once before child checks, but it must preserve tracked manifests, locks, source, and the exact frozen head/tree/Git status. Keep other writes, scripts, logs, and caches in external scratch; avoid competing setup or commands that mutate the shared frozen worktree.
...
- Review only introduced behavior in the frozen target against the compiled assignment, explicit repository rules, changed-code contracts, required checks, and demonstrated regressions. Suppress style, speculation, pre-existing defects, and unrelated improvement ideas. Plan V2 A rows and explicitly binding decisions govern requirements within Boundaries; the route is suggested. A routine fix satisfying acceptance and scope does not violate the plan merely by using another approach.
...
Read the A/V/T contract: distinguish observed baseline from required dependency guarantees and expected upstream edits, development feedback from acceptance proof, and final-only checks from prerequisite completion. Verify canonical toolchain owner/cwd/prerequisites/probes using scripts, pyproject/uv, Nix, manifests/locks, CI, and instructions, not `which` or an ambient binary. Reuse prepared dependencies. If declared dependencies or pinned assets are absent, perform repository-prescribed locked, source-preserving setup once before launching child checks, then rerun the probe; record exact setup cwd/command/result separately from check evidence. A subreviewer may do this only when its packet delegates sole setup ownership before concurrent checks. Require the exact frozen head/tree/Git status after setup. Never modify tracked manifests/locks/source, guess packages, use opportunistic unpinned `uvx`/`npx`, make global/system/privileged changes, inject credentials, weaken checks, or assume ambient HOME. The manager does not run automatic per-plan gates; main-session-selected final manifests alone become authoritative manager-run evidence.
...
Use background reviewers so the parent can run required shared gates once per frozen review target and process results as they arrive. Keep shared-gate ownership at the parent; subreviewers need not rerun the same suite.
...
After independently checking materiality, the parent reopens every surviving merged location, verifies its concrete scenario and introducing change, completes required shared gates and any unresolved mandatory work, classifies discovered paths, and applies the installed Reviewer contract's severity and relationship rules. Serious unresolved concerns or missing required checks/coverage remain incomplete, never approval.

Every final code-defect blocker requires an exact changed location, concrete triggering scenario, reproducible evidence or a failing check, and the introducing hunk/commit. A confirmed issue blocks only when it is an evidence-complete P0/P1 `PLAN_REQUIREMENT` or `PATCH_REGRESSION`, a failed explicit acceptance criterion, a failed required acceptance gate, or a material scope violation. Environment/invocation inability to run a check is handled separately below, never as defect evidence. Confirmed P2/P3 findings remain advisory. `FOLLOWUP` and `INVALID` never block. Preserve the contract's repair guidance and three-round authority rules. Round-1 evidence-complete blockers may authorize round 2 directly; only round-2 nonapproval not routed to ENVIRONMENT/INVOCATION attention invokes Judge, and Judge `REPAIR` authorizes ordinary review-driven round-3 rescue. Manager-proven operational failures or conflicts may advance to round 3 without prior Reviewer or Judge, using the unchanged original assignment and precise failure evidence, never an invented waiver. Exhaustion goes to the existing durable serialized main-session attention queue with a decision dossier of implemented/remaining work, checks and gaps, attempts, recommendation, and frozen identities. Review is independent: the rescuer cannot approve its own changes, and neither reviewer nor Judge may invent a fourth automatic mutation.

Before any terminal report, recheck frozen branch/HEAD/tree integrity; manager integrity checks precede blocker handling. A prerequisite still missing after permitted setup fails, unavailable setup authority, or wrong manager/argv/cwd is not a defect finding or scope failure. Return `BLOCK` with optional `BLOCKER_KIND: ENVIRONMENT` or `INVOCATION`, exact setup and check manager/command/cwd/result or error plus prerequisite/correction, without defect findings or SCOPE: FAIL. Report `SETUP` separately from `CHECKS`; an unrun check remains unrun. This routes to durable same-role/ready-phase, same-round `operator_attention` (`verification_environment`), not automatic code repair/retry; operator retries are explicit request-bound decisions. Missing plan/product authority may use `REQUIREMENT` for confirmed recovery/decision. Normal code outcomes and success omit the field. None of these classifications grants approval, mutation, waived tests, or acceptance of dirty/unreviewed changes. Never fabricate CHECKS; agent reports retain their provenance and cannot replace final manager evidence.
