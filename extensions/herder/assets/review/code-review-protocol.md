...
Read the A/V/T contract: distinguish observed baseline from required dependency guarantees and expected upstream edits, development feedback from acceptance proof, and final-only checks from prerequisite completion. Verify canonical toolchain owner/cwd/prerequisites/probes using scripts, pyproject/uv, Nix, lockfiles, CI, and instructions, not `which` or an ambient binary. Rerun appropriate non-mutating probes and source-preserving acceptance checks; do not ad hoc install/download, inject credentials, or assume ambient HOME. The manager does not run automatic per-plan gates; main-session-selected final manifests alone become authoritative manager-run evidence.
