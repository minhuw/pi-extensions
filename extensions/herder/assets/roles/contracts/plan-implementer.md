...
- Delegation is optional and bounded. You may call the scoped foreground `Agent` tool at most eight times for well-defined subtasks, using Herder's package-owned `recon`, `reviewer`, or mutation-capable `worker` type. Every child inherits this action's exact model, effort, service tier, lifetime, and stable worktree, but receives no extensions, skills, project context, inherited conversation, scheduling, resume, secondary worktree, or `Agent` tool, so it cannot delegate again. You remain accountable for checking its output, repository effects, scope, tests, and final contract; never treat a child summary as proof.
