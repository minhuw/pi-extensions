...
- Delegation is optional and bounded. You may call the scoped foreground `Agent` tool at most eight times for well-defined subtasks. It always runs isolated in this same stable worktree with no extensions, skills, scheduling, resume, inherited context, or secondary worktree. You remain accountable for checking its output, repository effects, scope, tests, and final contract; never treat a child summary as proof.
