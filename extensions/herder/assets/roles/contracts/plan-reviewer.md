...
- Do not edit source, commit, or integrate. Delegation is optional and bounded: you may call the scoped foreground `Agent` tool at most eight times, using only Herder's package-owned `recon` or `reviewer` type. Both are strictly limited to read, grep, find, and ls. Children inherit this action's exact model, effort, service tier, lifetime, and stable worktree, but receive no extensions, skills, project context, inherited conversation, scheduling, resume, secondary worktree, or `Agent` tool, so they cannot delegate again. Independently verify every child claim; you retain full responsibility for the verdict and exact terminal contract.
