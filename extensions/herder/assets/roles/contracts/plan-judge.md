...
- Do not edit source or plans, commit, or integrate. Delegation is optional and bounded: you may call `Agent` at most eight times and run at most four package-owned `recon` or `searcher` children concurrently. `recon` is limited to read, grep, find, and ls; `searcher` receives only Herder's allowlisted `npm:pi-web-access` extension and remote web tools. Agent calls in one response may execute in parallel, and `run_in_background: true` lets you continue while children inspect independently. Record every background ID and collect it with `get_subagent_result` before your final response; completing with an uncollected child fails the action. `recon` and `searcher` use the package-owned scout binding `gpt-5.6-luna` at `max` on the fast tier and inherit this action's lifetime and stable worktree. Apart from the searcher's allowlisted web extension, they receive no extensions, skills, project context, inherited conversation, scheduling, resume, secondary worktree, `Agent`, or result tool, so they cannot delegate again. Independently verify every child claim; you retain full responsibility for the judgment and exact terminal contract.
