...
Blocked-plan recovery, three-round exhaustion, and input waits use the existing durable serialized main-session attention queue. Substantive failures use `plan_recovery`, which may authorize a confirmed target-local `revise`; Judge questions use `user_decision`; exhausted provider or transport handling uses `operator_attention`, which never authorizes plan rewriting. Blocked worker envelopes may optionally report `BLOCKER_KIND: ENVIRONMENT | INVOCATION | REQUIREMENT`; normal code failures and success omit it. A prerequisite still absent after permitted declared setup fails, missing setup authority, or wrong invocation enters `operator_attention` (`verification_environment`) at the same role/ready phase and substantive round, without automatic retry or guessed code repair. Report `SETUP` and `CHECKS` separately with exact manager/command/cwd/result or error and prerequisite/correction; only explicit request-bound retry/cancel continues the blocked work. REQUIREMENT retains confirmed recovery/user-decision authority, never inferred scope expansion. Frozen review/Judge integrity checks precede blocker handling; dirty blocked implementation worktrees remain preserved and unreviewed changes cannot be accepted. On substantive exhaustion, the root receives implemented/remaining work, checks, attempts, recommendation, and frozen identities. Choices are: accept clean independently reviewed frozen changes only with explicit user confirmation and recorded gaps/waivers; revise the target into a new generation with prior evidence as history, not authority; stop while preserving branch/artifacts; or defer. Dirty, unreviewed, or in-progress-rebase surfaces cannot be accepted. Acceptance never fabricates passed tests or bypasses normal integration trust checks; separate final-run verification remains mandatory. Unrelated plans continue while dependents wait. A completed, parseable final Reviewer report without a classified blocker completes the original run. See [Attention, concurrency, and recovery](adapters/README.md#attention-concurrency-and-recovery) for request binding, interview and confirmation, continuation scheduling, and cleanup mechanics.

User-decision `answer` records the exact nonempty answer but leaves work **BLOCKED** (final RUN paused), requiring manual intervention even after resume. Use explicit `answer_and_resume` only when an in-scope clarification makes the unchanged assignment runnable; user-decision `retry` and legacy prose `user_input` cannot bypass this boundary. Upstream/scope/dependency revisions need separate user-invoked recovery, not worker scope expansion; see the [recovery limits](adapters/README.md#attention-concurrency-and-recovery).
