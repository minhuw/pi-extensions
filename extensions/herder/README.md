...
- A package-owned, one-level nested executor with up to four concurrent children per role; children inherit the parent action binding but cannot delegate again.
...
Each plan keeps one Herder-owned branch and worktree for its entire pipeline; workers and their direct nested children never create additional branches or worktrees. Nested children run through Herder's own executor, not the standalone subagents extension. Each role may run up to four children concurrently, including background children collected through `get_subagent_result`; Herder fails a role that returns with uncollected background work. Children inherit the parent action's exact model, effort, service tier, worktree, and cancellation lifetime; receive no skills, project context, inherited conversation, `Agent`, or result tool; and therefore cannot nest again. Reviewer and Judge may launch `recon` or the web-enabled `searcher`; Implementer may additionally launch `worker`. Extension discovery remains disabled for all children, but `searcher` loads the package-allowlisted `npm:pi-web-access` extension from Pi's trusted user package store when it has been explicitly installed with `pi install npm:pi-web-access`; project-scoped packages cannot shadow it. Concurrent children share the same stable worktree, so the parent remains responsible for partitioning and verifying their effects. The user's checkout remains unchanged until Herder performs its serialized integration step.
