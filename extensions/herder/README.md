...
All run control is user-invoked through the slash commands above. `/herder-cleanup` remains command-only, and the active model has no run-control tool. Ordinary cleanup preserves the integration branch/worktree, coordination refs, and plan directory; `--include-failed` additionally selects BLOCKED/REJECTED evidence. Plan-set-level `--deep` removes those preserved resources and the plan directory after a fail-closed preview that still requires a terminal run, reachable completion proofs, and merge ancestry. `--force` is the last-resort destroyer: it stops a live run if needed, then deletes the plan directory, owned worktrees, `herder/<plan>/` branches, and `refs/plan-herder/<plan>/` refs without those proofs. It still refuses to delete the current checkout. The removed `--finalize` and `--handoff-target` modes are rejected with guidance to use `--deep` or `--force`. The model-facing Herder surfaces are planning-only `herder_plan` (including request-bound attention resolution), request-bound `herder_verification`, request-bound `herder_integration_repair` (classification plus `begin`, `finish`, or `cancel` only), and request-bound `herder_reignite`; the adapter uses internal `herder_run` dispatch for manager operations and does not expose it as a model tool. The agentic planning commands inject the exact package-owned instructions and supplied arguments into the current Pi conversation, preserving the user's context. The instruction files remain private implementation assets, so each workflow has one public `/herder-*` command. `/herder-plans` is the fast deterministic surface: it parses typed subcommands and calls the native `herder_plan` application tool without spending a model turn. Mechanical `/herder-plans validate` and semantic `/herder-validate` are intentionally separate.

During an active Fire run, Improve, Simplify, and graph-wide planning mutations remain blocked. `/herder-grill --plan <id>` is allowed only when SQLite proves the target has never started. The manager reserves that plan so unrelated work continues, then stops new dispatches after the confirmed target-local edit, lets current workers settle, adopts a new immutable graph generation, and resumes automatically. Explicit `--split` fails before reservation with guidance to finish or stop Fire and run standalone. Outside Fire, ordinary `--plan` refinement may split when shaping requires it, and `--split` makes that intent explicit; Grill may update the target, directly affected siblings, index, dependencies, and shared context only as needed to preserve the decomposed objective and downstream guarantee. `/herder-revise` never authors or splits a standalone plan: it only adopts validated graph changes into an existing deterministic run.

Blocked-plan recovery and input waits are durable main-session attention requests. Substantive failures use `plan_recovery`; Judge questions use `user_decision`; exhausted provider or transport handling uses `operator_attention` and never authorizes plan rewriting. A parseable final Reviewer report never blocks the original run. The adapter presents one request at a time in deterministic plan-ID order through a typed follow-up message after the main session settles, while unrelated worker scheduling continues. Answers carry the request ID, immutable request hash, and capability token. Recovery keeps the Grill one-question and final-confirmation guarantees, permits a justified unchanged retry, and edits only the confirmed target plan. After an unchanged retry, validated replacement, or rejection, the manager force-cleans only the exact failed worktree and branch, preserves attempts, findings, usage, logs, and recovery history, and reschedules the recorded continuation; `/herder-resume` re-exposes unresolved requests after a reload.
...
After ordinary plans integrate, Herder asks the main Pi session to inspect the exact integration tree and submit a typed `herder_verification` manifest. The main session chooses commands but does not execute them. Gate working directories are TreeRelative (`"."` or a path inside the integration worktree); absolute paths are rejected. The manager validates the manifest's run, generation, assignment, head, tree, argv, and working directories, executes the gates in the background, and creates the final aggregate Reviewer only after they pass. If a gate fails, Herder preserves the evidence, keeps the UI alert, and injects a structured recovery turn with the exact log, failed head/tree, and a request-bound capability. Each failed verification request opens one immutable classification episode bound to its request/hash, integration head/tree, and canonical gate program. The main session submits exactly one classification for the current episode through `herder_integration_repair`: `manifest_error` replaces gates, `transient` retries the inherited gates once unchanged, `code_defect` opens a writable integration-repair transaction, and design/scope/credential/product ambiguity records a request-bound non-mutating user-decision outcome. A successor failure closes that episode and opens a fresh unclassified episode without changing the fixed-parent lineage or earlier audit history. Only an active transaction may edit failure-related integration paths; local diagnostics are optional and non-authoritative. For a code defect, the owning session stages and creates the next bounded repair commit or amends the logical repair commit while retaining the fixed parent, confirms a clean worktree, and submits its observed `HEAD`; Herder only validates the clean single-parent commit and reruns the retained authoritative gates. Accepted code commits are capped at three across all episodes; manifest corrections do not consume code rounds, and an unchanged transient retry cannot be reset by replaying identical head/tree/gate evidence. Additions to the ordered gate program must be explicit. `/herder-resume` remains operator recovery; ordinary deterministic integration defects no longer require graph revision, and the frozen integration tree is otherwise never edited in place.

The original run completes after any parseable final Reviewer report. Residual `PLAN_REQUIREMENT` and `PATCH_REGRESSION` findings become a new sibling plan directory named `herder-reignite`, then `herder-reignite-2`, and so on. The owning main session writes that directory in one shot with `herder_plan` `init`/`shape`/`validate` and acknowledges it with `herder_reignite`, passing the `graphSha256` returned by `validate`. `planDirectory` may be the source run or the allocated sibling; acknowledgement always targets the source run. `/herder-fire` of the allocated directory is a separate user command; a failed write leaves the original run complete and the dossier retryable.

Each plan keeps one Herder-owned branch and worktree for its entire pipeline; workers and their direct nested children never create additional branches or worktrees. Nested children run through Herder's own executor, not the standalone subagents extension. Each role may run up to four children concurrently, including background children collected through `get_subagent_result`; Herder fails a role that returns with uncollected background work. `recon` and `searcher` use the package-owned scout binding `gpt-5.6-luna` at `max` on the fast tier. `worker` inherits the parent action's exact model, effort, and service tier. Every child inherits the parent action's worktree and cancellation lifetime; receives no skills, project context, inherited conversation, `Agent`, or result tool; and therefore cannot nest again. Reviewer and Judge may launch `recon` or the web-enabled `searcher`; Implementer may additionally launch `worker`. Extension discovery remains disabled for all children, but `searcher` loads the package-allowlisted `npm:pi-web-access` extension from Pi's trusted user package store when it has been explicitly installed with `pi install npm:pi-web-access`; project-scoped packages cannot shadow it. Concurrent children share the same stable worktree, so the parent remains responsible for partitioning and verifying their effects. Attempt usage records keep parent tokens on the parent model and persist a nested breakdown by child type and model. The user's checkout remains unchanged until Herder performs its serialized integration step.
