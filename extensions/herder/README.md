...
Blocked worker envelopes may report `BLOCKER_KIND: ENVIRONMENT | INVOCATION | REQUIREMENT | SAFETY`. Environment/invocation failures preserve the same substantive round and require the specific external correction; they do not authorize guessed source repair. REQUIREMENT requests a precise decision, not expanded scope. An in-assignment SAFETY obstacle pauses for a decision without launching remediation; unrelated or pre-existing out-of-scope safety concerns are notify-only, never automatic blockers. Frozen review/Judge checks precede classification, and unfinished patches are never treated as reviewed. Report `SETUP` separately from `CHECKS`; interrupted or unrun checks remain unknown.
...
At exhaustion, user decisions remain explicit: a next round authorizes an exact bounded repair (scope and additional budget separately approved); accept-as-is records explicit exceptions, never fictitious passed checks or a final-gate waiver; drop preserves the patch and leaves dependent tasks blocked, not falsely complete. A decision report alone grants none of these permissions.
...
For ordinary code failures (not classified ENVIRONMENT/INVOCATION attention), each plan has at most **three substantive attempts**: initial implementation, guided repair, then fresh-context `RESCUE` using the existing Implementer and optional rescue binding. Every completed normal-mode Reviewer result goes to Judge, including approval, round 3, and final RUN audit. Judge classifies every finding; only an approved-contract violation or introduced/worsened bug may authorize an exact bounded repair. An explicit task to fix an existing bug is in scope. Pre-existing defects outside the assignment, unrelated safety concerns, and plan/design improvements are notifications, not blockers or new obligations. No fourth automatic mutation follows exhaustion. Judge `REPAIR` retains the existing bounded `PASS_DOCUMENT`, persisted in the terminal result and delivered with its actionId/hash, not a separate file. Manager-proven operational rescue may instead use precise failure evidence with `PASS_DOCUMENT: none`, never a waiver. Later review checks authorized repairs and introduced regressions rather than reopening broad discovery; missing first discovery still uses the Plan V2 risk floor. See [Three-round plan closure](adapters/README.md#three-round-plan-closure).

Compact round progress is rebuilt from persisted terminal `manager_actions`, grouped by run, plan, generation, and round, with the latest terminal action ID as its delivery dedup key. Existing NOTES/RATIONALE provide short done/checks/remaining summaries; SETUP and actual CHECKS remain worker self-reports, not authoritative passed gates. Interrupted evidence stays unknown. Reports include Judge-approved `fixNext` and cumulative `notIntendedToFix` exclusions. Deferred/rejected stable finding IDs cannot be relabeled NEW: reopening requires different explicit Reviewer evidence and Judge confirmation of a newly introduced/worsened regression. Exclusions are generation-scoped; user-approved changed contracts can reconsider scope. YOLO still shows Implementer summaries without fabricated reviews or judgments. Ordinary exact-tree final gates remain mandatory.
