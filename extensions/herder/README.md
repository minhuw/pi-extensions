...
Blocked-plan recovery, three-round exhaustion, and input waits use the existing durable serialized main-session attention queue. Substantive failures use `plan_recovery`, requirement/Judge questions use `user_decision`, and exhausted provider/transport handling uses `operator_attention`. For ordinary plans, these are evidence classifications, not different recovery menus: all delegate whole-run revision or explicit abandonment as described below. Blocked worker envelopes may optionally report `BLOCKER_KIND: ENVIRONMENT | INVOCATION | REQUIREMENT`; normal code failures and success omit it. A prerequisite still absent after permitted declared setup fails, missing setup authority, or wrong invocation enters `operator_attention` (`verification_environment`) at the same role/ready phase and substantive round, without automatic retry or guessed code repair. Report `SETUP` and `CHECKS` separately with exact manager/command/cwd/result or error and prerequisite/correction; ordinary-plan attention requires whole-run revision or abandonment, while final RUN retains explicit request-bound retry/cancel. REQUIREMENT never grants a worker inferred scope expansion. Frozen review/Judge integrity checks precede blocker handling; dirty blocked implementation worktrees remain preserved and unreviewed changes cannot be accepted. On substantive exhaustion, the root receives implemented/remaining work, checks, attempts, recommendation, and frozen identities to inform its whole-run revision proposal. Ordinary-plan attention no longer accepts incomplete work, retries unchanged, or defers as a resolution. Dirty, unreviewed, or in-progress-rebase surfaces cannot be accepted. Acceptance never fabricates passed tests or bypasses normal integration trust checks; separate final-run verification remains mandatory. Unrelated plans may continue until the whole-run revision barrier opens; then all workers settle. A completed, parseable final Reviewer report without a classified blocker completes the original run. See [Attention, concurrency, and recovery](adapters/README.md#attention-concurrency-and-recovery) for request binding, interview and confirmation, continuation scheduling, and cleanup mechanics.

### Whole-run revision after a blocked plan

When an ordinary plan escalates to attention, the main session inspects the failure and **proposes a whole-run revision by default**. Discuss and refine the proposal directly; there is no preliminary revise/defer questionnaire, unchanged retry, or acceptance of incomplete work. `herder_plan` attention `revise_run` opens a durable scheduling barrier and settles all workers before granting plan-graph Markdown editing. Any plan—including one already completed or integrated inside this run—may be rewritten, added, removed, or reordered, along with dependencies and shared context.
...
Approval adopts a new generation of the same run. Herder safely reverses invalidated contributions on the integration branch, preserving unrelated completed history, and deletes only invalidated execution surfaces. Conflicts or changed ownership keep the run blocked rather than silently expanding what is discarded. Final verification and final approval run again against the revised integration tree. The user's source checkout and branch are never reset; changed checkout/base or merged integration refuses recovery. A dismissed confirmation preserves the old execution and leaves the revision open, not retried. Explicit `abandon_run`, separately host-confirmed, discards the whole unmerged execution without restarting and preserves plan Markdown.

This policy applies to ordinary-plan recovery, requirement questions, and operator attention once escalated; internal bounded implementation/review rounds remain unchanged. Final **RUN** attention and exact-tree verification/integration repair retain their separate recovery contracts and budgets. In that final-only user-decision path, `answer` remains record-only and `answer_and_resume` requires a runnable within-scope clarification. See [Attention, concurrency, and recovery](adapters/README.md#attention-concurrency-and-recovery) for durable recovery details.
...
After ordinary plans integrate, Herder uses exact-tree verification and bounded recovery before the final Reviewer; see [Final verification and bounded recovery](adapters/README.md#final-verification-and-bounded-recovery) for the structured manifest, request-bound gate, and repair episodes. A completed, parseable final Reviewer report without a classified blocker completes the original run. Only eligible residual material blockers—P0/P1 `BLOCKING` findings with `PLAN_REQUIREMENT` or `PATCH_REGRESSION` relationship—enter automatic sibling `herder-reignite` drafting. Advisories remain in reports, not executable scope; see [Reignite write](adapters/README.md#reignite-write) for that handoff.
