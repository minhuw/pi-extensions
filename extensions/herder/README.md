...
Discovery and final audits use four parallel actual reviewers with explicit hunk/subsystem ownership and named cross-boundary questions. The parent owns assignment and shared preparation authority, completes any repository-declared source-preserving restoration before child checks, runs shared gates once, and independently verifies surviving merged claims; children reuse that prepared state and never run competing installs. Fresh targeted second opinions are optional. Reviewer bash is unrestricted, so source preservation is a behavioral contract, not a sandbox. See [Reviewer protocol](adapters/README.md#reviewer-protocol) for evidence, unresolved coverage, and bounded later verification.
