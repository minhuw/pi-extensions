...
After every replacement plan is TODO and the entire graph passes shape/validation, `finish_edit` presents host confirmation of the exact revision. Approval discards **all** old unmerged execution worktrees, branches, completion proofs, and runtime accounting, then automatically restarts every revised plan from the original run base with the same profile and parallelism. This version does not selectively reuse completed work. The user's source checkout and branch are never reset; changed checkout/base or merged integration refuses cleanup. A dismissed confirmation preserves the old execution and leaves the revision open, not retried. Explicit `abandon_run`, separately host-confirmed, discards the whole unmerged execution without restarting and preserves plan Markdown.
