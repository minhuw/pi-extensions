...
Discover each toolchain owner, canonical invocation, and source-backed setup from repository scripts, `pyproject.toml`/`uv.lock`, Nix declarations/locks, manifests/locks, and CI/instructions as applicable—not `which` or `command -v`. Verify cwd, version/availability probe, locked prerequisites, and evidence. Keep setup separate from checks and record whether workers may restore locked dependencies, pinned assets, or repository-prescribed setup; dependency selection and tracked manifest/lock changes require explicit bootstrap/feature scope. This read-only planning session performs no setup, downloads, credential injection, or ambient HOME workarounds. Record baseline observations and unrun checks honestly, without promising assets an earlier plan creates later. Missing preparation or a wrong invocation is not a code finding; investigate the declared environment first. Do not manufacture a standalone baseline/test plan for the same invariant.
