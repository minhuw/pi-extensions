...
**Handling rule:** never copy a secret value into a finding or plan — those files get committed. Reference the `file:line` and credential type only ("Stripe live key at `config.ts:12`"), and the fix sketch always includes rotation, not just removal (a committed secret is burned even after deletion). Rotation/revocation is urgent external operator work, not a Herder execution step or completion gate; keep repository remediation and its local proof separate, without claiming the credential is safe before rotation.
...
- Dependency posture: inspect declared manifests/locks and available advisories; use a repository-declared audit invocation only when known to be source-preserving through its canonical manager environment. Record any locked/pinned worker setup prerequisite, but this planning audit must not install/download an audit tool or substitute an ambient binary. Missing setup or wrong invocation is an operational gap, not a code finding. Report only critical/high advisories affecting reachable runtime code or build/distribution paths; avoid low-signal noise.
