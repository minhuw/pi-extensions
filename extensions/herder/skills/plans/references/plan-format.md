...
Command text is descriptive evidence/instruction: the parser never executes setup/checks or scrapes it into final gate argv. Implementers perform declared setup, pre-edit probes, and baseline diagnosis; Reviewers reuse or source-preservingly restore declared dependencies before independently rerunning appropriate acceptance checks. The manager does **not** automatically execute per-plan gates or add a preflight worker. Only the later request-bound final manifest is manager-executed authoritative verification. Agent `SETUP` and `CHECKS` retain self-report provenance, never runtime authority.
...
Discover invocations and setup from package scripts, `pyproject.toml`, `uv.lock`, Nix declarations/locks, manifests/locks, and CI/instructions as applicable. A declared uv project may prescribe locked sync before `uv run --no-sync ...`; a Nix project may prescribe its pinned `nix develop ... --command ...` environment. Record the exact source-backed setup authority separately in Prerequisites. Implementers may restore locked dependencies, pinned tool/browser assets, and repository-prescribed setup in their worktree using existing package-manager caches; Reviewers/Judges may repeat only source-preserving declared restoration without tracked manifest/lock/source edits. Dependency selection or tracked manifest/lock changes require explicit assignment authority, such as a bootstrap or feature dependency change. Never permit guessed packages, opportunistic unpinned `uvx`/`npx`, global/system/privileged changes, credentials, ambient HOME workarounds, weakened checks, or unrelated source fixes. Report setup cwd/command/result separately from checks. Structural `shape`/`validate` executes none of this and must not promise eager availability of dependencies/assets that an earlier bootstrap plan creates later.

Existing final GitDriver preparation remains a bounded npm-only mechanism: direct npm/npx gates with declared dependencies, no node_modules, and an npm lockfile get temporary `npm ci --ignore-scripts --no-audit --no-fund`, then created modules are removed. For another pinned asset needed under the gate's isolated HOME/cache, select an explicit canonical repository-owned setup-and-check invocation that performs preparation and the check within that one gate environment; each gate receives a fresh state root, so a prep-only gate cannot seed a later gate. If the repository has no such declared invocation, report the prerequisite instead of assuming Playwright or inventing generic orchestration. This is explicit evidence-backed invocation, not a toolchain auto-detection framework. Do not duplicate shared toolchain tables or maintain a separate executable config.

### Escalation and handoff

State plan-specific stop/decision triggers, the downstream invariant provided, safe intermediate state, and meaningful deferred work. Resolve uncertainty necessary to start **before writing**, not with a generic STOP escape hatch. Environment/invocation/setup failures are not code findings: report the exact manager, command, cwd, error, and missing prerequisite; do not guess a repair or broaden scope. Missing product authority routes to a decision. Never invent passed checks or infer runtime authority from `CHECKS`. Worker ENVIRONMENT/INVOCATION blockers use same-round operator attention, not automatic code repair. Once an ordinary plan escalates, the coordinator proposes whole-run revision by default; only revision or explicit abandonment resolves that plan attention. This does not authorize workers to change scope. Final RUN operator attention retains its separate retry/cancel contract. In final recovery, wrong invocation is `manifest_error`, proven prerequisite absence is non-mutating `environment`. Gate outcomes (`passed`, `command_failed`, `unavailable`, `timed_out`, `runner_error`) and error/timedOut/signal are process evidence only: a started uv/Nix wrapper missing a nested tool can be `command_failed`; exit 127 or log text alone never diagnoses a source defect.
...
Two unordered plans with equal declared write paths make `shapeReady=false`; the existing overlap report remains visible. Order them by dependency or reshape; overlap is not a harmless warning. Size/structural validity cannot prove semantic readiness or prepare dependencies. Before `shape` and `validate`, cold-read compiled snapshots, verify sources, A/V/T sufficiency, toolchain and setup evidence, starting guarantees, scope, and confirmed intent using the template's Producer self-review. Repeat after every local/shared edit.
...
The manager compiles initial lifecycle once and never rewrites README Status during ordinary execution. Under an explicitly reserved whole-run revision, the complete graph is editable, but unrelated completed plans need not be rewritten. Herder derives retained completion from immutable assignments and proofs, not authored statuses; changed plans and their downstream dependents are invalidated, unfinished execution restarts, and host approval activates the revised graph in a new generation with fresh final verification. A terminal snapshot is optional after complete/failed/stopped. Dependencies require canonical `DONE` plus a plan-set-scoped private completion ref naming a reachable commit. `IN PROGRESS` belongs to an active run; `BLOCKED` needs explicit direction. Respect all active-Fire reservations and request-bound recovery restrictions; structural validity is not edit authority.
