...
> Herder run; the deterministic Run Manager owns status transitions.
>
...
Written by a Herder plan producer and updated only by the deterministic Herder Run Manager during execution:
...
STOP conditions; only the deterministic Herder Run Manager updates status rows.
