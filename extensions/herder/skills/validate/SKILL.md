...
- reconcile caller/regression evidence with the snapshot, without requiring an audit ledger: necessary companion edits are in Boundaries write paths, preservation obligations in A requirements/Boundaries, and named coverage in acceptance V; unaffected read-only callers need no write scope; Implementation route anchors and A/V links separate binding decisions from suggestions;
...
- V phases have concrete commands/expected observations; acceptance covers named regressions and competing negative regressions, with expected assertion changes versus preserved behavior. Obsolete incidental assertions may migrate; never delete a security/behavior invariant just to pass. Label manual inspection honestly, not fake test proof; no mandatory broad searches/tests or full suite per plan;
- T owners, canonical invocations, cwd, prerequisites, non-mutating availability/version probes, and evidence follow repository scripts, pyproject/uv, Nix, manifests/locks, CI/instructions as applicable—not binary discovery; prerequisites name source-backed locked restoration, pinned assets, or repository-prescribed setup when applicable; absent future cwd/assets are justified by dependency guarantees rather than promised eagerly;
- setup is distinct from validation: this workflow executes none, while workers may perform only declared locked/pinned setup; dependency selection and tracked manifest/lock changes require explicit bootstrap/feature scope. Never authorize guessed packages, opportunistic unpinned uvx/npx, global/system/privileged changes, credentials, ambient HOME workarounds, weakened checks, or unrelated source repair; environment/invocation failures are not code findings;
...
Check that plans are coherent, independently testable invariants; distinct outcomes, ownership boundaries, caller cohorts, and public transitions split only at valid integration points. Keep tests/docs with their invariant unless a separately useful prerequisite justifies a node; avoid layer splitting. Before declaring ready, check caller/regression omissions, account for known coverage gaps in proof, and resolve material leads. Reconcile duplicates/overlaps and order equal write paths, including newly found shared writes and their dependency order; verify real dependencies and that each plan can begin from canonical integration HEAD using its snapshot and consumed guarantees. Missing necessary scope or proof is an `ERROR`; material scope/order choices need confirmation, not silent widening. No automatic manager per-plan gate/preflight phase exists: Implementer owns pre-edit probes and baseline diagnosis, Reviewer owns independent source-preserving acceptance checks, and the main session selects the separate manager-executed final manifest. Agent CHECKS never becomes authoritative final evidence.
