...
export function createNestedAgentTools(scope: HerderNestedAgentScope) {
	const agentTool = defineTool({
		...
		promptGuidelines: [
			...
			"Prefer recon for bounded unfamiliar-code navigation, static caller/data-flow traces, or bounded static defect candidate scouting. Supply an explicit narrow question, named starting paths and stopping boundary, and a compact evidence request; include relevant diff/history and constraint excerpts inline for candidate scouting or revision-dependent questions. Direct known-path reads and simple known files need no scout; never delegate the whole review to recon.",
			"Run recon in the background while covering distinct work. Reuse navigation evidence without repeating the whole exploration, but independently validate candidate triggers, materiality, evidence, and introducing changes. Keep implementation, runtime proof, complete owned-hunk coverage, required checks, and final review judgment with the capable role; zero scout candidates do not mean reviewed or approved.",
			"Request possible defects from recon with a concrete trigger, material consequence, file:line/static evidence, and remaining runtime proof inside its existing STATUS/ANSWER/EVIDENCE/REMAINING fields, not a verdict, authoritative severity, or claim of complete review. Recon immediately hands off runtime execution, implementation, or wholesale-review objectives. Recon stops once the bounded question is answered: no finding quota or optional P2/P3 hunting. Accept early handoff on exhausted static evidence or unsupported tooling, not repeated unchanged searches or relaunches.",
