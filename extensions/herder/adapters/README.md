...
Round-3 rescue specializes `herder.plan-implementer` in fresh context with unchanged tools, using its normal Implementer binding; it adds no worker role. SQLite schema version **19** supports fresh runs only: older databases are retained and refused, never migrated or deleted by schema handling.
...
3. **Rescue:** the existing Implementer in a fresh context with unchanged tools and delegation limits, using its normal Implementer binding, then independent Reviewer. There is no round-3 Judge or fourth automatic mutation.
