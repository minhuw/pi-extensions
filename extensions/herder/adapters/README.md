...
**Scoped nested delegation.** A Herder worker may delegate a bounded subtask only through the foreground `Agent` tool and remains accountable for inspecting, verifying, and integrating the result. The child always uses the parent's stable manager-owned worktree as its `cwd`; it does not create or switch to a secondary worktree. Every child is isolated from the Pi extension and skill environment, receives no inherited conversation, cannot schedule or background work, cannot resume a prior subagent session, is hidden from generic subagent result/steer/stop/resume surfaces, and has no Herder orchestration surface. Reviewer and Judge workers may delegate only to generic agent types that explicitly declare a read-only built-in tool set. Live nested status, activity, context, compaction, and usage telemetry appears only in the parent Herder TUI tree. It is ephemeral UI state: Herder does not persist it to manager SQLite and does not expose it on the dashboard. Parent worker completion removes the whole live nested tree.
