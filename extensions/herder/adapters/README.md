...
The live TUI state remains ephemeral. It is not persisted to manager SQLite or exposed on the dashboard; parent completion removes it after settlement. Terminal usage counts every descendant exactly once, keeping parent-session tokens on the parent model and a breakdown by nested type/model; the dashboard implementation card shows that breakdown. Parent and nested extension sessions bind noninteractively before prompting and emit `session_shutdown` before disposal.
