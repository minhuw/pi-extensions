...
- one scoped foreground-only `Agent` tool for bounded delegation.
...
**Scoped nested delegation.** Herder owns a narrow, one-level nested executor; it does not depend on the standalone `extensions/subagents` scheduler or telemetry bus. A Herder worker may delegate a bounded subtask only through its foreground, sequential `Agent` tool and remains accountable for inspecting, verifying, and integrating the result. The package-owned child types are `recon`, `reviewer`, and `worker`; Reviewer and Judge parents may use only the strictly read-only `recon` and `reviewer` types, while Implementer may also use `worker`. Each child inherits the parent action's exact model object, thinking level, service tier, stable manager-owned worktree, and cancellation lifetime. It receives a clean direct Pi SDK session with no custom `Agent` tool, extensions, skills, prompt templates, themes, project context files, inherited conversation, scheduling, background mode, resume, or secondary worktree, so a child cannot delegate again. Live child status appears as one flat level beneath the parent in Herder's TUI, and child token usage is added to the parent action's terminal evidence. The UI state remains ephemeral: Herder does not persist it to manager SQLite or expose it on the dashboard, and parent completion aborts and settles all remaining children before removing the tree.
