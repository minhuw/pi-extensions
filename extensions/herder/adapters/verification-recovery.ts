...
export const FINAL_VERIFICATION_SELECTION_GUIDANCE = [
	...
	"CANONICAL_INVOCATION: Verify repository scripts, pyproject/uv, Nix definitions, manifests/locks, CI, and instructions. Use the declared manager environment, not bare-binary discovery: for example uv run --no-sync only after verifying the prepared locked environment, nix develop --command for a declared pinned shell, or the canonical package script. Do not guess manager flags, packages, or assume a globally found binary is the project toolchain.",
	"ENVIRONMENT_BOUNDARY: Final gates run with a fresh minimal environment; interactive HOME configuration, ambient caches, and credentials are not inherited. Herder's npm-only locked node_modules preparation is narrow, not a general setup service. If a declared pinned tool/browser asset must be prepared under isolated HOME/XDG cache, select the repository-owned setup-and-check script as one direct-argv gate so setup and check share that private state and emit distinct log evidence. A prep-only gate cannot seed a later fresh gate. Do not assume Playwright, invent shell chains, sync/download unpinned tools, inject credentials, or create generic dependency orchestration.",
