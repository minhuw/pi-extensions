...
import { modelSupportsServiceTier, parseServiceTier } from "./service-tier.js";
...
export default function (pi: ExtensionAPI) {
  ...
  // When enabled, the three hardcoded default agents (recon, worker,
...
**Never delegate understanding.** Don't write "based on your findings, fix the bug" or "based on the research, implement it." Those phrases push synthesis onto the agent instead of doing it yourself. Write prompts that prove you understood: include file paths, line numbers, what specifically to change.`;
  ...
  pi.registerTool(defineTool({
    ...
    promptGuidelines: [
      ...
      "For broad codebase exploration, spawn Agent with subagent_type recon. For implementation use worker; for post-change review use reviewer. Otherwise use direct tools (read, grep, find) when the target is already known.",
    ...
    parameters: Type.Object({
      ...
      service_tier: Type.Optional(
        Type.String({
          description:
            'Optional service tier for OpenAI-compatible models: "fast"|"standard" (or raw "priority"|"default"|"flex"|"auto"). Frontmatter service_tier is authoritative when set.',
        }),
      ),
    ...
    execute: async (toolCallId, params, signal, onUpdate, ctx) => {
      ...
      const subagentType = resolved ?? "worker";
      ...
      // Validate service_tier tool param early (frontmatter already sanitized at load).
      if (params.service_tier != null && params.service_tier !== "") {
        try {
          parseServiceTier(params.service_tier);
        } catch (err) {
      ...
      // Normalize resolved service tier (frontmatter wins; tool param fills gap).
      let serviceTier = resolvedConfig.serviceTier;
      if (serviceTier != null) {
        try {
          serviceTier = parseServiceTier(serviceTier);
        } catch (err) {
          if (resolvedConfig.serviceTierFromParams) {
            return textResult(err instanceof Error ? err.message : String(err));
          }
      ...
      // Service tier only applies to OpenAI-compatible APIs. Params → hard error;
      // frontmatter pin → warn and proceed without tier.
      if (serviceTier && model && !modelSupportsServiceTier(model)) {
        const label = `${model.provider}/${model.id}`;
        if (resolvedConfig.serviceTierFromParams) {
          return textResult(
            `Model "${label}" (api: ${model.api ?? "unknown"}) does not support service_tier "${serviceTier}".`,
          );
        }
        ctx.ui.notify(
          `Agent "${customConfig?.displayName ?? subagentType}" service_tier "${serviceTier}" ignored — model ${label} does not support service tiers`,
          "warning",
        );
        serviceTier = undefined;
      }
      ...
      // "worker" may itself be unregistered (defaults disabled, no
      // user override) — getConfig then uses the hardcoded fallback config.
      const fallbackNote = fellBack
        ? `Note: Unknown agent type "${rawType}" — using ${resolveType("worker") ? "worker" : "the fallback agent config"}.\n\n`
        : "";
...
service_tier: <optional: fast|standard (or priority|default|flex|auto) for OpenAI-compatible models. Omit for provider default>
    ...
    function buildItems(): SettingItem[] {
      ...
      return [
        ...
        },
        {
          ...
          description: "Hide built-in agents (recon, worker, reviewer) — custom agents are unaffected",
