...
import { parseServiceTier } from "./service-tier.js";
import type { AgentConfig, MemoryScope, ServiceTierInput, ThinkingLevel } from "./types.js";
 ...
 * Any name is allowed — names matching defaults (e.g. "recon") override them.
 */
...
function loadFromDir(dir: string, agents: Map<string, AgentConfig>, source: "project" | "global"): void {
  ...
  for (const file of files) {
    ...
    agents.set(name, {
      ...
      serviceTier: parseServiceTierField(fm.service_tier),
...
/** Parse service_tier frontmatter; invalid values are ignored (agent still loads). */
function parseServiceTierField(val: unknown): ServiceTierInput | undefined {
  try {
    return parseServiceTier(val);
  } catch {
    return undefined;
  }
}
