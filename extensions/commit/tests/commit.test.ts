...
import { assertSafeGitEnvironment, guardedGitArguments, sensitivePathKind } from "../preflight.ts";
...
mkdirSync(path.join(fakeRepositoryPath, ".git"));
...
function safeDirtyExec(calls?: Array<{ command: string; args: string[] }>) {
	return async (command: string, args: string[]): Promise<ExecResult> => {
		...
		if (args.includes("diff")) return { stdout: "+++ b/src/index.ts\n+export const value = 1;\n", stderr: "", code: 0 };
...
test("dirty trusted worktrees pass redacting preflight and dispatch through the user lifecycle", async () => {
	...
	assert.deepEqual(calls, [
		...
		{ command: "git", args: guardedGitArguments(fakeRepositoryRoot, ["ls-files", "--others", "--exclude-standard", "-z"]) },
		{ command: "git", args: guardedGitArguments(fakeRepositoryRoot, ["diff", "--no-ext-diff", "--no-textconv", "--unified=0", "--no-color"]) },
		{ command: "git", args: guardedGitArguments(fakeRepositoryRoot, ["diff", "--cached", "--no-ext-diff", "--no-textconv", "--unified=0", "--no-color"]) },
	]);
...
test("redacting scan catches secrets on removed diff lines without reproducing values", async () => {
	...
	const pi = piFor({
		exec: async (_command, args) => {
			const identity = fakeIdentityResult(args);
			if (identity) return identity;
			if (args.includes("status")) return { stdout: " D src/config.ts\0", stderr: "", code: 0 };
			if (args.includes("ls-files")) return { stdout: "", stderr: "", code: 0 };
			if (args.includes("diff") && !args.includes("--cached")) return { stdout: `--- a/src/config.ts\n+++ /dev/null\n-const key = "${secret}";\n`, stderr: "", code: 0 };
			return { stdout: "", stderr: "", code: 0 };
		},
	});
	await assert.rejects(() => launchCommitWorkflow(pi, context()), (error: unknown) => {
...
test("redacting scan catches secrets in unchanged context of a dirty tracked file", async () => {
	...
	try {
		const pi = piFor({
			exec: async (_command, args) => {
				const identity = fakeIdentityResult(args, root);
				if (identity) return identity;
				if (args.includes("status")) return { stdout: " M config.ts\0", stderr: "", code: 0 };
				if (args.includes("ls-files")) return { stdout: "", stderr: "", code: 0 };
				if (args.includes("diff")) return { stdout: "+++ b/config.ts\n+const changed = true;\n", stderr: "", code: 0 };
				return { stdout: "", stderr: "", code: 0 };
			},
		});
		await assert.rejects(() => launchCommitWorkflow(pi, context({ cwd: root })), (error: unknown) => {
	...
	try {
		...
		const pi = piFor({
			exec: async (command, args) => {
				if (args.includes("rev-parse") && args.includes("--verify") && args.includes("--quiet") && ++quietRefChecks === 2) {
...
test("oversized staged diffs cannot be marked reviewed or committed", async () => {
	...
	try {
		...
		await gitTool!.execute("status-staged", { operation: "status" }, undefined, undefined, ctx);
		await assert.rejects(
			() => gitTool!.execute("diff-staged", { operation: "diff", scope: "staged" }, undefined, undefined, ctx),
			/Git diff is too large for complete guarded review/,
		);
		await assert.rejects(() => gitTool!.execute("commit", {
			operation: "commit",
			subject: "commit: reject incomplete diff review",
			body: "Require the exact staged tree to fit within complete guarded review.",
...
test("killed staged diff output cannot satisfy exact-tree review", async () => {
	...
	try {
		...
		const pi = piFor({
			exec: async (command, args) => {
				if (killDiff && args.includes("diff") && args.includes("--cached") && !args.includes("--unified=0")) {
		...
		await assert.rejects(
			() => gitTool!.execute("diff-staged", { operation: "diff", scope: "staged" }, undefined, undefined, ctx),
			/git diff was interrupted or timed out/,
		);
		killDiff = false;
		await assert.rejects(() => gitTool!.execute("commit", {
			operation: "commit",
...
test("post-preflight Git races invalidate the run before tool execution", async () => {
	...
	const pi = piFor({
		exec: async (command, args) => {
			...
			if (args.includes("ls-files")) return { stdout: "", stderr: "", code: 0 };
			if (args.includes("diff")) return { stdout: changed ? "+++ b/src/other.ts\n+changed\n" : "+++ b/src/index.ts\n+changed\n", stderr: "", code: 0 };
			return { stdout: "", stderr: "", code: 0 };
		},
		onUserMessage: (message) => { dispatched = message; },
		onCommand: (_name, command) => { registered = command; },
